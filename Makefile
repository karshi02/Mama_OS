...
BUILD_DIR = build
CFLAGS = -m32 -ffreestanding -fno-stack-protector -Wall -Wextra -Iinclude -Idrivers -std=gnu99 -O2 -fno-pie -no-pie -mgeneral-regs-only
...
debug: $(FINAL_IMG)
	$(QEMU) -drive format=raw,file=$< -m 256M -s -S -no-reboot -no-shutdown
